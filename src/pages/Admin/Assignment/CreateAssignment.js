...
const CreateAssignment = (props) => {
  ...
  useEffect(() => {
    dispatch(setHeaderTitle('Manage Assignment > Create Assignment'));
  }, [dispatch]);
    ...
    if (currentUser) {
      checkIfDisabledUser(currentUser.code).then((res) => {
        if (res.data.data) {
          ...
          createNewAssignment(selectedUser.value, selectedAsset.value, dateAssigned, note, currentUser.code)
            .then((res) => {
            ...
            .catch((error) => {
              ...
              if (code.includes('ASSET')) {
                showStoreErrorMessage(error, selectedAsset.name, dispatch);
              } else if (code.includes('USER')) {
                showStoreErrorMessage(error, selectedUser.name, dispatch);
              } else {
                var message = ERRORS[code];
                if (message) {
                  message = format(message, selectedAsset.name, selectedUser.name);
                } else {
                  message = code;
                }
  ...
  return (
    <div className='container'>
      <h3 className='text-org font-bold text-base mb-5'>Create Assignment</h3>
      <div className='mt-6'>
        <form className='mb-0 grid-cols-3 inline-grid space-y-4 w-full lg:mr-16' onSubmit={handleSubmit(addAssignment)}>
          ...
          <div className='flex items-center col-span-2 relative w-full'>
            ...
            >
              <div
                ...
                style={{ width: '35rem' }}
              >
                <div className='flex items-center justify-between w-auto h-full'>
                  <h3 className='text-org font-bold text-lg mb-5'>Select User</h3>
                  <div className='relative mr-3'>
                    <input
                      type='search'
                      name='search'
                      className='pr-10 pl-5 border w-full rounded-md focus:outline-none focus:border-gray-500 focus:ring-gray-500 border-gray-600'
                              ...
                              index > 0 && (
                                ...
                                >
                                  <div className='flex flex-col'>
                                    <div className='flex items-center justify-start h-10 xl:h-6 w-max whitespace-nowrap'>
                                      {header.label}
                        ...
                        >
                          <td className='px-3'>
                            <input
                              ...
                              checked={selectedUser.value === item.code}
                              onChange={(e) =>
                                setSelectedUser({
                                  ...selectedUser,
                    ...
                    {users && users.length > 0 && (
                      <button
                        ...
                        className={`text-white p-1.5 rounded-md w-20 ${
                          !selectedUser.value ? 'bg-gray-500 cursor-default' : 'bg-org-light hover:bg-org focus:bg-org'
                        }`}
                  ...
                  <div className='flex justify-end items-center gap-5 pt-7'>
                    {assets && assets.length > 0 && (
                      <button
                        type='button'
                        className={`text-white p-1.5 rounded-md w-20 ${
                          !selectedAsset.value ? 'bg-gray-500 cursor-default' : 'bg-org-light hover:bg-org focus:bg-org'
                        }`}
          ...
          <div className='flex items-center col-span-2'>
            <div className='input-field'>
              <textarea draggable={false} className='resize-none' defaultValue={''} {...register('note')} rows={4} />
            </div>
          </div>
          <div></div>
